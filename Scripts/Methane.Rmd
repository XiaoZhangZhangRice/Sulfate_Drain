...
flux_2025_rice_interpolated <- flux_2025_rice_interpolated %>%
  ...
  mutate(CH4_flux_g_ha_day = CH4_flux_corrected*240 )%>%
  ungroup()
...
```{r}
methane_all_graphing_3 <- methane_all_graphing %>% filter(Treatment !="LD-MS") %>% filter(Treatment !="CF-PP")

methane_over_time_3 <- 
  ggplot(methane_all_graphing_3, aes(x = Date, y = CH4_flux_corrected,  colour =  Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=CH4_flux_corrected-CH4_flux_corrected_se, ymax=CH4_flux_corrected+CH4_flux_corrected_se), size=0.5)+
  ggtitle("Methane over time")+
...
ggsave(methane_over_time_3, filename = "../Figures/methane_over_time_3trt.jpg", height = 25, width = 30, units = "cm", dpi=400)
```
...
```{r}
model <- lm(total_CH4_emissions_kg_ha~Treatment, data=flux_2025_rice_seasonal_emissions%>%filter(Treatment != "CF-PP")%>%filter(Treatment !="LD-MS"))
...
Treatment_means = emmeans(model,spec = 'Treatment')
Treatment_effects = contrast(Treatment_means, method = 'pairwise', adjust = "none")
summary(Treatment_effects)
cld(Treatment_means)
