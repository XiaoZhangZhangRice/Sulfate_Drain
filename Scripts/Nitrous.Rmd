...
```{r}
nitrous_all_graphing_3 <- nitrous_all_graphing %>% filter(Treatment !="LD-MS") %>% filter(Treatment !="CF-PP")

nitrous_over_time_3 <- 
  ggplot(nitrous_all_graphing_3, aes(x = Date, y = N2O_flux_corrected,  colour =  Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=N2O_flux_corrected-N2O_flux_corrected_se, ymax=N2O_flux_corrected+N2O_flux_corrected_se), size=0.5)+
  ggtitle("Nitrous Oxide over time")+
  scale_x_datetime(name = "",
    limits = as.POSIXct(c("2025-05-01", "2025-09-11")), # Set x-axis limits
    date_labels = "%b %Y",
    date_breaks = "1 month")+
  scale_color_manual(values = c("CF" = "#0072B2", "LD" = "#E69F00", "LD-PP" = "#7B6BAF"),
                     labels = c(expression("Continuous Flood (CF)"), 
                                expression("Drain"),
                                expression("Drain and Sulfate")
                                )
                     )+
...
ggsave(nitrous_over_time_3, filename = "../Figures/nitrous_over_time_3trt.jpg", height = 25, width = 30, units = "cm", dpi=400)
```
