...
# Do calculations manually to make sure flux values are correct (this is for excel based checking)

```{r}
library(Peak2Flux2)

final_flux_std <- final_flux %>% 
  filter(Sample_Type == "Std")%>%
  select(Std_CH4_Peak,	Std_CH4_PPM,Std_N2O_Peak,Std_N2O_PPM,"GC_Run ") 

final_flux_std$Std_CO2_Peak <- NA
final_flux_std$Std_CO2_PPM <- NA

final_flux_std$Std_Gas1_PPM <- NA
final_flux_std$Std_Gas1_Peak <- NA

final_flux_std$Std_Gas2_PPM <- NA
final_flux_std$Std_Gas2_Peak <- NA

final_flux_std$Std_Gas3_PPM <- NA
final_flux_std$Std_Gas3_Peak <- NA


names(final_flux_std) <- trimws(names(final_flux_std))

final_flux_sample <- final_flux %>% 
  filter(Sample_Type == "Sample")

final_flux_sample <- final_flux_sample %>%
  mutate(
    Sample_CH4_ppm = NA,
    Sample_N2O_ppm = NA,
    Sample_CO2_Peak = NA,
...
final_flux_sample$Sample_CH4_Peak <- final_flux_sample$CH4_Sample_Peak
final_flux_sample$Sample_N2O_Peak <- final_flux_sample$N2O_Sample_Peak

names(final_flux_sample) <- trimws(names(final_flux_sample))

str(final_flux_sample)
str(final_flux_std)


final_data <- peak2ppm(final_flux_std, final_flux_sample)

write.xlsx(
  final_data,
  "../Data/Flux_Calc/sample_ppm_values.xlsx",
  overwrite = TRUE
)
```
