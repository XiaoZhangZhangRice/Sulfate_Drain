...
```{r}
sulfate_all <- read_excel("../Data/Porewater/Sulfate + Drain IC Porewater Sampling Data Seasonal Final.xlsx", sheet = 1)
str(sulfate_all)
```
...
```{r}
model <- lmer(Sulfate_Concentration_ppm~Drain*Sulfate+(1|Plot)+(1|Blk)+(1|Date_Collected), data=sulfate_all)
...
model <- lmer(Sulfate_Concentration_ppm~Treatment*as.factor(Date_Collected)+(1|Plot), data=sulfate_all)
pls205_diagnostics(model)
anova(model)
...
# Soil sampling dates
  ...
  geom_text(aes(x = as.POSIXct("2025-06-17"), 
            ...
            angle = 90, vjust = -0.5, hjust = 0, color = "brown", size =6) +
  # Soil sampling dates
...
```{r}
sulfate_all_graphing_4trt <- sulfate_all_graphing %>% filter(Treatment != "LD-MS") 


sulfate_over_time_4trt <- 
  ggplot(sulfate_all_graphing_4trt, aes(x = Date_Collected, y = Sulfate_Concentration_ppm,  colour =  Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=Sulfate_Concentration_ppm-Sulfate_Concentration_ppm_se, ymax=Sulfate_Concentration_ppm+Sulfate_Concentration_ppm_se), size=0.5)+
  ggtitle("(b) Pore water sulfate concentration")+
  scale_x_datetime(name = "",
  ...
  labs(y = expression("Sulfate concentration (mg L"^-1~")")) +
  theme_classic()+
...
# pre season sulfate    
  ...
  geom_text(aes(x = as.POSIXct("2025-05-01"), 
                y = mean(Sulfate_Concentration_ppm +100, na.rm = TRUE) ,
                label = "Pre-season sulfate"),
...
ggsave(sulfate_over_time_4trt, filename = "../Figures/sulfate_over_time_4trt.jpg", height = 25, width = 30, units = "cm", dpi=400)

```
## 3 treatment dataset with soil sampling dates

```{r}

sulfate_over_time_3trt_soils <- 
  ggplot(sulfate_all_graphing_3trt, aes(x = Date_Collected, y = Sulfate_Concentration_ppm,  colour =  Treatment))+
  geom_point()+
  geom_line()+
  geom_errorbar(aes(ymin=Sulfate_Concentration_ppm-Sulfate_Concentration_ppm_se, ymax=Sulfate_Concentration_ppm+Sulfate_Concentration_ppm_se), size=0.5)+
  ggtitle("Sulfate over time")+
  ...
  labs(y = expression("Porewater sulfate concentration (mg L"^-1~")")) +
  theme_classic()+
# pre season sulfate    
  geom_vline(xintercept = as.POSIXct(c("2025-05-01")), 
             linetype = "dashed", color = "#5F8A8B") +
...
# End of season drain     
  ...
  geom_text(aes(x = as.POSIXct("2025-06-17"), 
                y = mean(Sulfate_Concentration_ppm+100, na.rm = TRUE) ,
                label = "Pre Drain soils"),
            angle = 90, vjust = -0.5, hjust = 0, color = "brown", size =5) +
  # Soil sampling dates
  geom_vline(xintercept = as.POSIXct(c("2025-07-14")), 
             linetype = "dashed", color = "brown") +
  geom_text(aes(x = as.POSIXct("2025-07-14"), 
                y = mean(Sulfate_Concentration_ppm+100, na.rm = TRUE) ,
                label = "Post Reflood soils"),
            angle = 90, vjust = 1.4, hjust = 0, color = "brown", size =5) +
...
ggsave(sulfate_over_time_3trt_soils, filename = "../Figures/sulfate_over_time_soils_3trt.jpg", height = 25, width = 30, units = "cm", dpi=400)
